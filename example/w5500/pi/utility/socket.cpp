...
/**
 * initialize the channel in perticular mode, and set the port and wait for
 * w5500 done it.
...
int16_t recv(SOCKET s, uint8_t *buf, int16_t len)
{
	...
	if (ret == 0) {
		...
		if (status == SnSR::LISTEN || status == SnSR::CLOSED ||
			...
			// The remote end has closed its side of the connection, so this is
			// the eof state
			ret = 0;
		} else {
			// The connection is still up, but there's no data waiting to be
			// read
			ret = -1;
		}
	} else if (ret > len) {
 ...
 * Unlike TCP transmission, The peer's destination address and the port is
 * needed.
 *
	...
	if (((addr[0] == 0x00) && (addr[1] == 0x00) && (addr[2] == 0x00) &&
		...
		while ((w5500.readSnIR(s) & SnIR::SEND_OK) != SnIR::SEND_OK) {
			if (w5500.readSnIR(s) & SnIR::TIMEOUT) {
				...
				w5500.writeSnIR(s,
				                (SnIR::SEND_OK |
				                 SnIR::TIMEOUT)); /* clear SEND_OK & TIMEOUT */
				return 0;
			}
		}
 ...
 * is used to receive UDP, IP_RAW and MAC_RAW mode, and handle the header as
 * well.
 *
 * return received data size for success else -1.
 */
